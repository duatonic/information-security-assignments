...
class DES:
    ...
    def generate_subkeys(self):
        key = self.permute(self.str_to_bin(self.key), PC1)
        subkeys = []
        l, r = key[:28], key[28:]
        for i in SHIFT:
            l = l[i:] + l[:i]
            r = r[i:] + r[:i]
            subkeys.append(self.permute(l + r, PC2))
        return subkeys

    def str_to_bin(self, s):
        return ''.join(format(ord(c), '08b') for c in s).ljust(64, '0')
    ...
    def expand(self, block):
        return self.permute(block, EXPANSION)
    ...
    def sbox(self, block):
        ...
        for i, block in enumerate(blocks):
            ...
            col = int(block[1:5], 2)
            result += format(SUB_BOXES[i][row][col], '04b')
        return result
    
    def f_function(self, block, subkey):
        block = self.expand(block)
        block = self.xor(block, subkey)
